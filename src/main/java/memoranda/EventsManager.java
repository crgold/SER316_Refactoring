...
import main.java.memoranda.TimeData.Day;
...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	public static boolean isNREventsForDate(CalendarDate date) {
		Day d = TimeData.getDay(date, _root);
		if (d == null)
			return false;
	...
	public static IEvent createEvent(
		...
		Day d = TimeData.getDay(date, _root);
		if (d == null)
			d = TimeData.createDay(date, _root);
	...
	public static Collection getRepeatableEventsForDate(CalendarDate date) {
		...
		for (int i = 0; i < reps.size(); i++) {
			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
				getRepeatableEventsForDate(date, v, ev);
			}
		}
		return v;
	}

	private static void getRepeatableEventsForDate(CalendarDate date, Vector v, IEvent ev) {
		if (ev.getRepeat() == REPEAT_DAILY) {
			...
			if ((n - ns) % ev.getPeriod() == 0 || date.getCalendar().get(Calendar.DAY_OF_WEEK)
					== ev.getPeriod() || date.getCalendar().get(Calendar.DAY_OF_MONTH)
					== ev.getPeriod())
				v.add(ev);
		} 
	...
	//Class is too big so I moves all the static classes that dealt with years, months and days 
	//into a new class called TimeData and then refractored the code here to use it.
